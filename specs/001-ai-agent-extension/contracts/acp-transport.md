...
3. After a successful handshake, NDJSON ACP framing begins via the SDK's stream
   utilities.
...
- `session/new` — one ACP session per agent run; the extension's global `Conversation`
  maps to a sequence of ACP sessions (a new session starts when the conversation is
  cleared).
- `session/prompt` — carries user text plus page-content chunks produced by `readPage`;
  the agent MAY return tool calls, which the extension executes against the active tab
  using the same [tool contracts](./tools.md) and reports back.
