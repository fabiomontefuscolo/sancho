...
- [x] T020 [US2] Wire connection-failure handling (`chat.error` with actionable message, conversation preserved; ACP disconnect sets `AgentSession.state = "error"`) in entrypoints/background.ts
...
- [x] T049 [P] Restricted-page degradation E2E on chrome:// and web store pages in tests/e2e/restricted-pages.spec.ts
- [x] T050 Run full quickstart.md validation (all 5 scenarios) and fix any failures
...
- [x] T054 Amend spec.md with functional requirements for agent time awareness and the ACP MCP tool bridge, which were user-requested but are absent from the FR list (unrequested)
- [x] T055 Surface ACP `requestPermission` calls to the user with approve/deny instead of auto-cancelling in src/providers/acp.ts per FR-006 (partial)
